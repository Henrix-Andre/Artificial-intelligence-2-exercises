...
def checarAptidao(individuo):
    ...
    if peso > lPM or peso == 0:
        return False
    #mochila vazia não é considerada apta
...
def crossover(populacao, pai, mae):
    ...
    while completo == False:
        gene = random.randint(1, 4) #escolhe o gene que vai passar, sem pegar o zero
        pos = math.ceil(len(populacao) / gene)
        if gene == 4: #se for o 4 só faz a troca normalmente
            temp = f1[gene]
            f1[gene] = f2[gene]
            f2[gene] = temp
        else:
    ...
    #joga os dois novos individuos na população.
    populacao.append(f1)
    populacao.append(f2)

def dizimar(populacao,maxpop):
    min = 999 #min e max são os valores minimo e maximo pra saber quem vai ser retirado.
    max = best[1]
...
def bestCheck(populacao):
    ...
    for i in range(len(populacao)):
        ...
        if checarValor(populacao[i]) > best[1]:
...
def geneticRun(epochs, populacao, maxpop):
    ...
    for i in range(epochs):
        ...
        matchmaking(populacao)
        dizimar(populacao,maxpop)
        bestCheck(populacao)
        epochsUsed = epochsUsed + 1
        if best[2] == 80:
          break
...
def main():
    populacao = genPop(100)
    geneticRun(100,populacao,100)
